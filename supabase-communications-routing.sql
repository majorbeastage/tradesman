-- ============================================================
-- Communications routing + event logging
-- Run after:
...
CREATE UNIQUE INDEX IF NOT EXISTS client_communication_channels_public_address_key
  ON public.client_communication_channels (public_address);

CREATE INDEX IF NOT EXISTS client_communication_channels_user_id_idx
...
CREATE INDEX IF NOT EXISTS communication_events_user_id_idx
  ON public.communication_events (user_id, created_at DESC);

CREATE INDEX IF NOT EXISTS communication_events_conversation_id_idx
...
COMMENT ON TABLE public.client_communication_channels IS 'Live routing config for Twilio/Resend numbers or inboxes per client/user.';
COMMENT ON TABLE public.communication_events IS 'Inbound and outbound communication audit trail, including voicemail transcript/summary data.';
-- Communications routing + inbox foundation
-- Run in Supabase SQL Editor after supabase-profiles-roles.sql and supabase-office-manager-rls.sql.
...
CREATE TABLE IF NOT EXISTS public.client_communication_channels (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  UNIQUE (provider, public_address)
);
...
COMMENT ON TABLE public.communication_events IS 'Normalized inbound/outbound communication log for unread state, voicemail transcripts, and future mobile notifications.';
