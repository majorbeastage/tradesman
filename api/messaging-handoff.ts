...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (action === "issue") {
      ...
      const code = `mh_${randomBytes(32).toString("base64url")}`
      const expiresAt = new Date(Date.now() + CODE_TTL_MS).toISOString()
      const { error } = await service.from("messaging_handoff_codes").insert({
        user_id: user.id,
    ...
    if (action === "redeem") {
      ...
      // Conditional update is the single-use gate. Only one concurrent redeemer can win.
      ...
      if (linkError || !tokenHash) throw linkError ?? new Error("Could not create Messaging session")
    ...
    const message = error instanceof Error ? error.message : "Messaging handoff failed"
