/**
 * Server-safe org peer helpers for Vercel (no ../src / AuthContext imports).
 * Keep in sync conceptually with src/lib/organizationPeers.ts.
 */
...
function resolveInternalMemberLabel(profile: {
  ...
  if (meta && typeof meta === "object" && !Array.isArray(meta)) {
    const raw = (meta as Record<string, unknown>).contact_first_name
    if (typeof raw === "string") first = raw.trim()
  }
...
export { resolveInternalMemberLabel }
