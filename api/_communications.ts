...
async function ensureEmailChannelForPlatformRoute(
  ...
  if (route.route_kind === "department" && route.channel_id) {
    ...
    if (
      ...
      return deptChannel as CommunicationChannel
    }
  }
  ...
  if (route.route_kind === "department") {
    ...
    if (primaryRoute?.channel_id) {
      ...
      if (primaryCh?.id) {
        ...
        return primaryCh as CommunicationChannel
      }
    }
  }

  if (route.route_kind !== "customer_primary") return null
  ...
  if (route.channel_id) {
    ...
    if (linked?.id) {
      const ch = linked as CommunicationChannel
      if (
        ch.active &&
  ...
  if (userChannel?.id) {
    ...
    return { ...(userChannel as CommunicationChannel), public_address: publicAddress, forward_to_email: forwardTo, email_enabled: true, active: true }
  }
  ...
  return inserted as CommunicationChannel
}

/**
 * Explains why inbound Resend mail did not route (mirrors supabase/functions/_shared/inbound-email-channel.ts).
 */
