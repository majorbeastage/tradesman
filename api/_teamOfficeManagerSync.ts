/** Keep Admin Users ↔ MyT Team members in sync via office_manager_clients + team_member_invites. */
...
/** Assign or remove office-manager link and mirror team_member_invites (Admin + MyT stay aligned). */
...
/** Drop stale accepted invites; ensure every office_manager_clients row has a matching accepted invite. */
...
export async function loadLinkedTeamMemberIds(sb: SupabaseClient, accountOwnerId: string): Promise<string[]> {
  const { data: links, error: linkErr } = await sb
    .from("office_manager_clients")
    .select("user_id")
    .eq("office_manager_id", accountOwnerId)
