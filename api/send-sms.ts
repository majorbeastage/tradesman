...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (accountSid && authToken && fromNumber) {
      ...
      if (userId) {
        try {
          ...
          console.error("[send-sms] Twilio OK but communication_events insert failed:", logErr instanceof Error ? logErr.message : logErr)
    ...
    return res.status(500).json({
      ...
      message: msg,
      hint: "Check Vercel env SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, and Twilio credentials.",
    })
  }
}
