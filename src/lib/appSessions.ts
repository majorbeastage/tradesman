 ...
 *
 * TEMPORARILY DISABLED: register/heartbeat/revoke RPCs were failing in production and
 * interfering with platform login + generating Supabase errors. Device tracking is
 * kept as local prefs only until the SQL functions are verified healthy.
 */
...
/** Kill-switch: set true only after `supabase/user-app-sessions.sql` is confirmed on prod. */
...
export async function revokeLocalAppSession(
  ...
  /* disabled — never block sign-out on session RPC */
}
