...
export default function TermsPage() {
  const [content, setContent] = useState<SimpleLegalPage>({ ...DEFAULT_TERMS_PAGE })
    ...
    void (async () => {
      try {
        const { data, error } = await supabase
          ...
          .select("value")
          .eq("key", TERMS_SETTINGS_KEY)
          .maybeSingle()
        if (!error && data?.value) setContent(parseSimpleLegalPage(data.value, DEFAULT_TERMS_PAGE))
      } finally {
        setLoading(false)
      }
    })()
